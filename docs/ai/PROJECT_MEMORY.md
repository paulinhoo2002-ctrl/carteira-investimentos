...
## V272 trusted cash-flow/performance foundation — 2026-09-26 (local validation complete; PR pending)

- Worktree: `C:/Projetos/carteira-investimentos.worktrees/v272-cashflow-performance-foundation`; branch `feature/v272-cashflow-performance-foundation`; base `4fae02ac06729aea1c66a6008b24bba75b9a68b2`. Local implementation and QA are complete. Commit/push/PR status should be read from Git/GitHub after this snapshot; no merge is authorized.
...
- Final validation after fail-closed ambiguous-flow regression fix, scoped UI contrast adjustment and lockfile install: focused classifier/V271/V248/history/runtime + inline syntax 93/93; `npm run test:modern` 815/815; `npm test` 249/249; modern/legacy builds PASS; `npm run qa:all` PASS (QA harness 2/2 and seven viewport smoke). Isolated rendered Rentabilidade QA confirmed the V272 section at all seven widths with no horizontal/panel overflow or runtime/console error. Scoped axe on `.v248-history-panel`: 0 violations. Whole legacy page still reports nine pre-existing contrast findings outside this V272 component. Screenshots were visually inspected; QA used synthetic localhost `testMode`, not the real authenticated portfolio.
...
- PR #412 (V262) was squash-merged as `9be3d9c3e17b659a507e46a8f57c2b152174366b`; V263 branched from that merge on `feature/v263-rf-freshness-valuation-asof`. Its 2026-09-25 certification checkpoint was later squash-merged as `346ae421222f5f167d7ad2ce2c62cd7ed2639e41`.
...
- Visual QA correction (2026-09-25): old screenshots showing browser-default/un-styled HTML did not represent the application because the QA method did not load the built CSS/JS correctly; Vercel captures showed only the SSO gate. The supported local method is `npm run build:modern` followed by `vite preview --config modern/vite.config.ts` and opening `/host.html`, then navigating to the dedicated readonly Renda Fixa view. On this method both V262 base `9be3d9c3e17b659a507e46a8f57c2b152174366b` and V263 load Inter/theme/CSS, keep page width equal to viewport at 390 and 1366, and use responsive mobile cards. No page-level overflow regression was observed; the desktop data table itself has its own horizontal scrolling wrapper.
- Final local compiled-app QA on V263 HEAD: 390/430/768/1366/1440/1536/1920 all mounted with page width equal to viewport; axe-core 4.13.0 reported 0 WCAG A/AA violations; screenshots `V263-final-rf-390.png` and `V263-final-rf-1366.png` were visually reviewed, plus a 768/1920 review capture. Preview origin remained protected by Vercel SSO, so this evidence is local compiled demo/fixture QA, not authenticated real-portfolio preview QA. No financial or tax writes were performed.
- Runtime limitation observed identically on V262 base and V263: the existing IPCA SGS request to `api.bcb.gov.br` returns HTTP 400 `Invalid initial date` for `dataInicial=07/2022`. The fetcher is unchanged by V263; this is a pre-existing V262 source/request issue and is not evidence for changing V263 financial-as-of semantics. Keep it visible as an outstanding product/data-source issue until handled in a scoped follow-up.
- At V263 certification time, fresh worktrees needed `build:modern` before browser smokes; V265 subsequently moved this prerequisite into the integrated QA command and fixed static-helper 404 behavior.
- Gates re-run on V263 HEAD: modern 805/805, general 249/249, `build:modern` PASS, legacy `build` PASS, `git diff --check` PASS. FINANCIAL_WRITE_COUNT=0 and TAX_WRITE_COUNT=0 by scope. PR #413 was subsequently merged.
...
- Exact-head CI run `36079088319` succeeded. Vercel deployment `6651218341` is READY for that SHA. Authenticated QA used the explicitly authorized branch preview alias and the product route `/?protectedReadOnlyQa=1`.
...
- Offline snapshot root cause: protected QA correctly blocked canonical `civ5`/cloud writes, but offline eligibility was coupled to an old canonical snapshot. Active-wallet selection only changed in-memory state, so an existing zero-asset wallet could be recovered instead of the selected real wallet. The fix uses a separate, user-bound, schema-versioned local read-only snapshot for the selected wallet; the canonical `civ5` fingerprint remains unchanged. Snapshot is generated only from authenticated app-loaded state, with provenance/as-of and validation; invalid or wrong-user snapshots fail closed. `LOCAL_OFFLINE_CACHE_WRITE != FINANCIAL_WRITE` when confined to this local recovery cache. A cold profile still requires legitimate online authenticated hydration first. Never fabricate or inject snapshot data.
- Service-worker lesson: after deployment, an already-open QA tab can keep the previous worker active. Wait for and activate the product's legitimate update before testing offline; otherwise results describe the stale shell, not the deployed SHA.
