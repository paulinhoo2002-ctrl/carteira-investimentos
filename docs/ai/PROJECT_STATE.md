...
- No IPCA+ automatic security valuation, financial formula, manual fixed-income authority, persistence, or financial/tax write behavior changed. Local final validation: modern 777/777, general 249/249, modern and legacy builds PASS, diff-check PASS. CI/preview evidence is pending the V264 PR.
- PR #414 remains open/unmerged; see live GitHub state before acting. No merge was performed.
...
- PR [#413](https://github.com/paulinhoo2002-ctrl/carteira-investimentos/pull/413): OPEN and mergeable; CI and Vercel preview were verified green on the implementation head and are revalidated on the final certified head during certification.
...
- V263 implementation head under final visual certification (2026-09-25): `f3ed317a1ede6caee41e8bd0ba083be2045156bb`; PR #413 OPEN/mergeable. Exact CI run `36133823828` SUCCESS; Vercel deployment `dpl_7PrFSfsjY73PD9fC3ijaS4cxU42g` READY. Re-run gates: modern 805/805; general 249/249; modern and legacy builds PASS; diff-check PASS. Correct compiled local preview served `host.html` and CSS/JS assets; both V262 base and V263 were styled and had no page-level overflow at 390/1366. Seven local fixture widths (390, 430, 768, 1366, 1440, 1536, 1920) passed; axe-core WCAG A/AA 0 violations. Screenshots were reviewed locally. Vercel preview requires per-origin SSO; authenticated real-portfolio QA was not completed in this visual pass. No merge was authorized/executed.
...
- The 24-column table scrolls inside its own wrapper; page-level overflow `1886px` cited in older notes was not reproduced in either correctly served V262 or V263 build and is removed as an unverified QA artifact. Existing SGS IPCA API request still returns 400 `Invalid initial date` for `07/2022` in both base and V263; it is a pre-existing V262 request/data-source issue, not a V263 change.
