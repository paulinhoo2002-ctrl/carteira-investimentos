...
const overviewPagePath = require('node:path').join(__dirname, '..', 'modern', 'src', 'features', 'overview', 'OverviewPage.tsx');
...
async function loadModules() {
  const mod = await import(pathToFileURL(overviewPagePath).href);
  OverviewPage = mod.OverviewPage;
...
test('OverviewPage usa totalValue real no card Patrimônio', async () => {
  ...
  assert.ok(html.includes('R$ 1.000,00'));
});
...
test('próximos dividendos usam generatedAt como referência', async () => {
  ...
  const futureIncome = makeIncomeSnapshot({
    generatedAt: '2026-07-15T12:00:00.000Z',
    items: [
      { id: '1', ticker: 'PETR4', name: 'Petrobras', type: 'Dividendo', paymentDate: '2026-07-14', competenceDate: null, receivedValue: 100, taxValue: null, quantity: null, note: '', source: 'demo', sourceEventKind: null, sourceEventId: null },
