...
const sourceFiles = [
  ...
  'src/features/reports/reportsSnapshotAdapter.d.ts',
  ...
  'src/domain/fixedIncome/fixedIncomeRateParser.ts',
  'src/domain/fixedIncome/fixedIncomeWeekdays.ts',
  'src/features/fixed-income/fixedIncomeReadonlySupplementBuilder.ts',
  'src/features/fixed-income/fixedIncomeAssetIdentity.ts',
  'src/domain/fixedIncome/cdiContractParser.ts',
  'src/domain/fixedIncome/cdiRateEngine.ts',
  'src/domain/fixedIncome/cdiDailyFactorProvider.ts',
  'src/types/navigation.mjs',
  'src/types/shellNavigation.mjs',
  'src/types/shellNavigation.d.ts',
  ...
  'src/bootstrap/hostGoalsReadonlySource.ts',
  'src/bootstrap/modernGoalsRuntime.ts',
  'src/features/goals/GoalsReadonlyPage.tsx',
  'src/features/goals/GoalsReadonlyPage.css',
  'src/features/goals/goalsReadonlyBridge.mjs',
  'src/features/goals/goalsReadonlyBridge.d.ts',
  'src/features/goals/goalsReadonlyContract.mjs',
  'src/features/goals/goalsReadonlyContract.d.ts',
  'src/features/goals/goalsRefreshController.ts',
  'src/features/goals/goalsSnapshotAdapter.mjs',
  'src/features/goals/goalsSnapshotAdapter.d.ts',
  'src/features/goals/legacyGoalsReadonlyIntegration.ts',
  'src/features/goals/readonlyGoalsViewModel.ts',
];
...
test('modern shell exists and stays isolated', async () => {
  ...
  assert.equal(packageJson.scripts.build, "node -e \"const fs=require('fs'); const files=['index.html','manifest.json','sw.js','v250-runtime.js','v250-offline-session.js']; for (const f of files) { if (!fs.existsSync(f)) { throw new Error('Missing file: ' + f); } } console.log('Build OK: static app validated.');\"");
