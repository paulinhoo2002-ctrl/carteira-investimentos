...
export interface ReadonlyFixedIncomeViewModelWithValuation {
  readonly query: string;
  readonly selectedSubtype: string;
  readonly sortBy: ReadonlyFixedIncomeSortKey;
  readonly categories: readonly string[];
  readonly filteredItems: readonly ReadonlyFixedIncomeValuationItem[];
  readonly topLiquidItems: readonly ReadonlyFixedIncomeValuationItem[];
  readonly topProfitItems: readonly ReadonlyFixedIncomeValuationItem[];
  readonly topLossItems: readonly ReadonlyFixedIncomeValuationItem[];
  readonly topMaturityItems: readonly ReadonlyFixedIncomeValuationItem[];
...
function formatFreshness(value: 'FRESH' | 'STALE' | 'UNKNOWN') {
  switch (value) {
    case 'FRESH':
      return 'Atual';
    case 'STALE':
...
function formatValuationStatus(value: ValuationState['authoritativeStatus']) {
  switch (value) {
    case 'MANUAL_AUTHORITATIVE':
      return 'Manual';
    case 'SHADOW_AVAILABLE':
      return 'Shadow disponível';
    case 'SHADOW_PARTIAL':
      return 'Shadow parcial';
    case 'SHADOW_STALE':
      return 'Shadow desatualizado';
    case 'UNAVAILABLE':
      return 'Indisponível';
    case 'NEEDS_REVIEW':
...
function formatComparabilityStatus(value: 'COMPARABLE' | 'NOT_COMPARABLE') {
  return value === 'COMPARABLE' ? 'Comparável' : 'Não comparável';
}
...
export function createReadonlyFixedIncomeViewModel(
  ...
  const topLiquidItems = sortItems(
    enrichedItems.filter((item) => typeof item.liquidValue === 'number' && Number.isFinite(item.liquidValue)),
    'liquidValue',
  ).slice(0, 3);
  const topProfitItems = sortItems(
    enrichedItems.filter((item) => typeof item.profitValue === 'number' && item.profitValue > 0),
    'profitValue',
  ).slice(0, 3);
  ...
  const distribution = createSubtypeDistribution(enrichedItems);
